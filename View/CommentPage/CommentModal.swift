...
struct CommentModal: View {
    ...
    @State var commentToPost: Comment = Comment()
    ...
    func postComment() {
        commentToPost.content = self.content
        if let token = TokenManager.shared.getToken() {
            commentVM.postComment(accessToken: token, mapId: cafeVM.cafeMapId, commentToPost: commentToPost)
        }
    }
}
