...
struct MyCommentList: View {
    ...
    var body: some View {
        VStack {
            ...
            //목록
            List {
                ForEach(myVM.myCommentCafeArray, id: \.self) { cafe in
                    CommentedCafeCard(cafe: cafe)
                        .padding(.top)
                        .padding(.vertical, 14)
                }
                if let isEnd = myVM.myCommentData.isEnd, isEnd == false {
                    Button(action: {
                        loadMoreList()
                    }, label: {
        ...
        .onAppear {
            ...
            if let token = TokenManager.shared.getToken() {
                myVM.fetchMyComment(accessToken: token, page: 0)
            }
        }
    }
    ...
    @ViewBuilder
    func CommentedCafeCard(cafe: Cafe) -> some View {
        HStack {
            //바로가기 버튼
            NavigationLink(destination: {
                ...
                VStack(alignment: .leading) {
                    ...
                    //댓글 목록
                    VStack(alignment: .leading) {
                        ForEach(cafe.commentContents ?? [], id: \.self) { content in
                            //카드 하나
                            HStack(spacing: 13) {
                                VStack(alignment: .leading, spacing: 3) {
                                    Text(content)
    ...
    func loadMoreList() {
        if let token = TokenManager.shared.getToken(), let isEnd = myVM.myCommentData.isEnd, isEnd == false {
            myVM.loadMoreList(accessToken: token, tab: .comment)
        }
    }
}
